...
# Initialize positions, velocities, and forces for N_protons, N_neutrons and N_electrons
def initialize_positions():
    N_particles = N_protons + N_neutrons + N_electrons
    positions = np.random.rand(N_particles, 3) * L
    return positions

positions = initialize_positions()
initial_positions = positions.copy()
velocities = np.random.randn(N_particles, 3) * 1e-2  # Initialize with small random velocities
...
for step in range(steps):
    ...
    # Debugging: Print positions and velocities every 10000 steps
    if step % 10000 == 0:
...
plt.subplot(1, 2, 1)
plt.scatter(initial_positions[:N_protons, 0], initial_positions[:N_protons, 1], s=1, c='red', label='Protons')
if N_neutrons > 0:
    plt.scatter(initial_positions[N_protons:N_protons+N_neutrons, 0], initial_positions[N_protons:N_protons+N_neutrons, 1], s=1, c='green', label='Neutrons')
plt.scatter(initial_positions[-N_electrons:, 0], initial_positions[-N_electrons:, 1], s=1, c='blue', label='Electrons')
plt.xlim(0, L)
plt.ylim(0, L)
plt.xlabel('x')
plt.ylabel('y')
...
plt.legend()
plt.savefig('results/initial_positions.png')

plt.subplot(1, 2, 2)
plt.scatter(positions[:N_protons, 0], positions[:N_protons, 1], s=1, c='red', label='Protons')
if N_neutrons > 0:
    plt.scatter(positions[N_protons:N_protons+N_neutrons, 0], positions[N_protons:N_protons+N_neutrons, 1], s=1, c='green', label='Neutrons')
plt.scatter(positions[-N_electrons:, 0], positions[-N_electrons:, 1], s=1, c='blue', label='Electrons')
plt.xlim(0, L)
plt.ylim(0, L)
plt.xlabel('x')
plt.ylabel('y')
...
plt.legend()
plt.savefig('results/final_positions.png')
